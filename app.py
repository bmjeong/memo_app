...
def admin_screen():
    ...
    if "admin_target_date_last" not in st.session_state:
        st.session_state.admin_target_date_last = today

    if "admin_target_date" not in st.session_state:
        st.session_state.admin_target_date = today

    if st.session_state.admin_target_date_last != today:
        st.session_state.admin_target_date = today
        st.session_state.admin_target_date_last = today

    if "admin_list_date_last" not in st.session_state:
        st.session_state.admin_list_date_last = today

    if "admin_list_date" not in st.session_state:
        st.session_state.admin_list_date = today

    if st.session_state.admin_list_date_last != today:
        st.session_state.admin_list_date = today
        st.session_state.admin_list_date_last = today
...
def child_screen(child_name):
    if child_name == "율하":
        ...
        if "selected_date_율하_last" not in st.session_state:
            st.session_state.selected_date_율하_last = today

        if "selected_date_율하" not in st.session_state:
            st.session_state.selected_date_율하 = today

        if st.session_state.selected_date_율하_last != today:
            st.session_state.selected_date_율하 = today
            st.session_state.selected_date_율하_last = today

    elif child_name == "서하":
        show_otter_header(flg="bear")
        if "selected_date_서하_last" not in st.session_state:
            st.session_state.selected_date_서하_last = today

        if "selected_date_서하" not in st.session_state:
